...
string Joueur::m_a_j_etat(const Joueur& j_adv)
{
    ...
    if(i>= taille)
        return "echec emat";
  while(j < j_adv.taille)
    {
        if(j_adv.liste_pieces[j]!=0 && j_adv.liste_pieces[j]->verifer_deplacement(liste_pieces[i]->Getpos())&&
           j_adv.liste_pieces[j]->verifer_chemain(liste_pieces[i]->Getpos(),j_adv.liste_pieces,j_adv.taille,liste_pieces,taille) )
        {
           break;
        }
        else j++;
...
void Joueur::afficher(/*SDL_Surface *ecran*/) const
{
...
void Joueur::initialiser_pieces()
{
    ...
    liste_pieces[8] =  new Tour (Position(7,0),couleur);
    pos.Setx(0);
    pos.Sety(0);
    liste_pieces[9] = new Tour (pos,couleur);
...
*/
int Joueur::chercher(Position p)const
{
    for(int i=0;i<taille;i++)
        if(liste_pieces[i]!= 0 && liste_pieces[i]->Getpos() == p)
            return i;
    return -1;
}
